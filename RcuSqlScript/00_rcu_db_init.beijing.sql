...
DROP TABLE IF EXISTS RCU_WIND_RECORD;
DROP TABLE IF EXISTS RCU_SETTING_TEMP_RECORD;
DROP TABLE IF EXISTS RCU_CARD_RECORD;
...
CREATE TABLE RCU_WIND_RECORD (
room_id int not null,
date_time DATETIME not null ,
room_temp int not null default 1,
PRIMARY KEY (room_id,date_time),
FOREIGN KEY (room_id) REFERENCES RCU_ROOM_STATE(room_id) );

CREATE TABLE RCU_SETTING_TEMP_RECORD (
room_id int not null,
date_time DATETIME not null ,
room_temp int not null default 23,
PRIMARY KEY (room_id, date_time),
...
BEGIN
    ...
    END IF;
    IF NEW.setting_temp <> OLD.setting_temp THEN
       INSERT INTO RCU_SETTING_TEMP_RECORD(room_id,date_time,room_temp) VALUES (NEW.room_id,now(),NEW.setting_temp);
    END IF;
    IF NEW.wind_level <> OLD.wind_level THEN
       INSERT INTO RCU_WIND_RECORD(room_id,date_time,room_temp) VALUES (NEW.room_id,now(),NEW.wind_level);
    END IF;
    IF NEW.card <> OLD.card THEN
       INSERT INTO RCU_CARD_RECORD(room_id,date_time,room_temp) VALUES (NEW.room_id,now(),NEW.card);
    END IF;
